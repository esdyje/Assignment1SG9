...
clean_data %>%
  group_by(clean_data$ID) %>%
  summarise(M=mean(clean_data$CHI_MLU)) %>%
  filter(M >2.7)
...
datavisit = clean_data %>% group_by(visit) %>% mutate (Mean_words_v= sum(tokens_CHI)/length(visit))

#2
DatawCHItoken= clean_data %>% group_by(ID) %>% mutate(mean_token_CHI = sum(tokens_CHI)/length(tokens_CHI))
