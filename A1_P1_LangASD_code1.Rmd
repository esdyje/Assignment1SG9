---
title: "Assignment 1, Portfolio 1"
...
library(stringr);library(tidyverse);library(plyr);library(dplyr)
```
...
```{r}
dat1= read.csv("demo_train1.csv")
dat2 = read.csv("LU_train.csv")
dat3 = read.csv("token_train.csv")
```
...
dat1=select(dat1, ID, visit, Ethnicity, Gender, Age, Diagnosis, ADOS, MullenRaw, ExpressiveLangRaw)
dat2=select(dat2, ID, visit, MOT_MLU, MOT_LUstd, CHI_MLU, CHI_LUstd)
dat3=select(dat3, ID, visit, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
...
Visit1_data=select(Visit1_data, ID, ADOS, nonVerbalIQ, verbalIQ)
...
clean_data %>%group_by(ID) %>%summarise(M=mean(CHI_MLU)) %>%filter(M >2.7)

# MLU of less than 1.5 morphemes on first visit
df2=filter(clean_data, visit == 1 & CHI_MLU < 1.5)

# have not completed all trials.
missed = clean_data %>% group_by(ID) %>% tally()
missed = filter(missed, n < 6)
```
...
Visit_6x=arrange(Visit_6, c(Visit_6$tokens_CHI))
...
Visit_1x=arrange(Visit_1, c(Visit_1$tokens_CHI))
...
ASD_data2=select(ASD_data, ID, ID, visit, CHI_MLU, tokens_CHI)
...
```{r}
#1
MeanTokenVisit = clean_data %>% group_by(visit) %>% mutate (Mean_words_VISIT = sum(tokens_CHI)/length(visit))

#2
MeanTokenChild= clean_data %>% group_by(ID) %>% mutate(mean_token_CHI = sum(tokens_CHI)/length(tokens_CHI))
