...
plot = ggplot(Visit1_data, aes(x = Diagnosis, y = Age)) + geom_violin()
plot
...
plot1 = ggplot(Visit1_data, aes(x = Diagnosis, y = nonVerbalIQ)) + geom_violin()
plot1
...
plot2 = ggplot(Visit1_data, aes(x = Diagnosis, y = verbalIQ)) + geom_violin()
plot2

```

Reporting results of models:
Age: there is a significant difference, ASD children are significantly older than TD children (b=-12.63 , se = 1.02 , t-value(60) = -12.32 , p<0.001).
Gender: results suggest that gender has a nonsignificant influence on the diagnosis (b=-0.36 , se = 0.7 , z-value(60) = -0.52  , p= 0.6)
Non-verbal IQ: XXXXXX (b=-0.89 , se = 1.17 , t-value(60) = -0.76 , p=0.45).
Verbal IQ: XXXXXXXX 
...
#Random factors is ID and Visit
...
There is a significant difference in how the mothers' MLU changes over time, 
...
```{r}
bestmodel=lmer(CHI_MLU~visit+Ethnicity+verbalIQ+(1+visit|ID),data, REML=FALSE)
summary(bestmodel)
