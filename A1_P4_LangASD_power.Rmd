...
# model1wfakedata = lmerTest::lmer(CHI_MLU~1+visit*Diagnosis+(1+visit|ID),datawfake, REML=FALSE) # could do this with existing data.

model2wfakedata = lmerTest::lmer(CHI_MLU~1+visit+Diagnosis+(1+visit|ID),datawfake, REML=FALSE)
#setting interesting effectsizes for model 1 (interaction Model)
fixef(model2wfakedata)["visit"] <- 0.05
fixef(model2wfakedata)["DiagnosisTD"] <- -.10

summary(model2wfakedata)

#make power curve for the new data including the fake data
powerCurvewfakeV = powerCurve(model2wfakedata, fixed("visit"),along="ID", nsim=100) 
